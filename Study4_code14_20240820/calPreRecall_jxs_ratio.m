function [precision_1,precision_2,precision_3,precision_4,precision_5,recall_1,recall_2,recall_3,recall_4,recall_5,accuracy_1,accuracy_2,accuracy_3,accuracy_4,accuracy_5,specificity_1,specificity_2,specificity_3,specificity_4,specificity_5,f1_measure_1,f1_measure_2,f1_measure_3,f1_measure_4,f1_measure_5,mcc_1,mcc_2,mcc_3,mcc_4,mcc_5,youden_1,youden_2,youden_3,youden_4,youden_5,NDCG_1,BP,aupr,aucpre,auc_mroc,auc_groc] = calPreRecall_byl(score_list, train_list, test_list)
    %修改 train_list：对于所有的负样本（即 train_list 和 test_list 中都为 0 的位置），设它们的数量为 n，从中随机抽取 n?test 个边，并将它们标记为 train_list 中的正样本（即设置为 1）。
    %这样可以保证负样本的数量等于test中链路的数量
    negative_samples_idx = find(train_list == 0 & test_list == 0);  % 找到负样本的索引
    num_test_samples = sum(test_list == 1);  % 计算test_list中正样本的数量
    num_negative_samples_to_add = length(negative_samples_idx) - num_test_samples; %trian_list要从negative sample中添加的数量
    % 确保抽取的数量不为负
    if num_negative_samples_to_add > 0
        % 从负样本索引中随机抽取 num_negative_samples_to_add 个样本
        random_negative_samples = randsample(negative_samples_idx, num_negative_samples_to_add);
        % 把随机抽取的负样本加入 train_list 中
        new_train_list = train_list;
        new_train_list(random_negative_samples) = 1;
    else
        % 如果没有足够的负样本需要加入，则返回原始 train_list
        new_train_list = train_list;
    end
    train_list=new_train_list;
    non=find(train_list == 0 & test_list == 0);
    score_list1=score_list;
    scores = score_list(train_list == 0); %只留下(U-ET)行（也就是EP+negative_samples），用于计算ut
    score_list = score_list + 0.0001; %keep the zero candidate edges
    score_list = score_list.*~train_list; %去除训练集中已有的连边，只保留(U-ET)
    % 在所有可能的连边(|U|=n*(n-1)/2)中剔除ET的连边,即：得到(U-ET)中连边的得分值(~train_list表示全集U中除了训练集外的全部）
    % 寻找score_list中除训练集以外的行数及数值大小(score_list中除了训练集以外没有0，最少都是0.0001）
    % 也就是将全集U中除了训练集以外的所有链路(U-ET)的位置和分值提取出来
    [row, ~, weight] = find(score_list); % 所有的(U-ET)的位置(row)&预测分值(weight)
    [~, y] = sort(weight,'descend');     % 将(U-ET)得分向量按照降序从大到小排序(取值越大,y对应的排名越小);y-排名值，等价于idx
%    y(1):weight中最大值在weight中的位置；y(length(weight))=y(candidate_len):weight中最小值在weight中的位置；
%    Nnetwork=nnz(Net)/2;
%    Ntrain=nnz(train_list);
%    Ntest=nnz(test_list);

    candidate_len = length(weight);      % (U-ET)的大小 全部的正样本
...
%     precision=pre_list(ceil(candidate_len));
    ...
    [scores,~] = sort(-scores, 'ascend'); %idx是将链路按照打分从大到小排序
    [~,ut,~] = unique(scores); % scores 中唯一值的第一次出现的索引
    ut = [ut(2:end)-1; length(weight)]; % scores 每个唯一值的结束索引
